...
<body style="font-family: 'Bebas Neue', serif;">
    ...
    <section>
        <div class="container py-4 py-xl-5">
            ...
            <?php
            $DesignerCoreQuery = "SELECT * FROM listings WHERE core = 3 ORDER BY created_at DESC";
            $DesignerCoreResult = $con->query($DesignerCoreQuery);

            // Check if there are listings
            if ($DesignerCoreResult->num_rows > 0) {
                // Open the row div outside of the while loop
                echo '<div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3">';

                while ($row = $DesignerCoreResult->fetch_assoc()) {
