...
<body style="font-family: 'Bebas Neue', serif;">
    ...
    <section>
        <div class="container py-4 py-xl-5">
            ...
            <?php
            $AvantGardeCoreQuery = "SELECT * FROM listings WHERE core = 2 ORDER BY created_at DESC";
            $AvantGardeCoreResult = $con->query($AvantGardeCoreQuery);

            // Check if there are listings
            if ($AvantGardeCoreResult->num_rows > 0) {
                // Open the row div outside of the while loop
                echo '<div class="row gy-4 row-cols-1 row-cols-md-2 row-cols-xl-3">';

                while ($row = $AvantGardeCoreResult->fetch_assoc()) {
